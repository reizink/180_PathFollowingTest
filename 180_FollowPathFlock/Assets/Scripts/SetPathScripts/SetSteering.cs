...
[System.Serializable]
public class SetSteering : MonoBehaviour
{
    public List<Transform> path; //fill in waypoints
    ...
    // Note: if the follower bounces around the same point,
    // your points are either too far apart or the FollowDist is too small
    private Vector3 FollowLeader()
    {
        ...
        // the "?" means that the value is 'nullable' or able to be null
        ...
        {
            float distance = Vector3.Distance(futurePosition, path[i].position);
            if (distance < closestDistance)
            {
                closestDistance = distance;
                closestPoint = path[i].position;
                closestIndex = i;
            }
        }

        if (closestPoint == null) return Vector3.zero; // No valid point found, avoid null errors
        ...
        Vector3 futurePathPoint = path[predictedIndex].position;
        ...
        else if (closestIndex < path.Count - 1)
        {
            ...
            return Seek(path[closestIndex + 1].position);
        }
